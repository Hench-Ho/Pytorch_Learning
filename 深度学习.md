...
[(3 封私信 / 44 条消息) 终于懂了！从零实现 GPT tokenizer (以 BPE 为例) - 知乎](https://zhuanlan.zhihu.com/p/714899440)
...
`manifest` 的结构具体依赖于数据集的需求，但通常它包括以下几个字段：

1. **id**: 唯一标识符，通常是音频文件或数据条目的标识符。
2. **audio**: 音频文件的路径或 URI，通常是指向音频文件的完整路径。
3. **duration**: 音频的时长，以秒为单位。
4. **text**: 转录文本，表示音频文件中说话的内容。
5. **speaker** (可选): 如果是多说话人数据集，可能会包含说话人的信息。
6. **language** (可选): 语言信息，表示该数据集的语言。
7. **features** (可选): 预先提取的特征（如 Mel-spectrogram 或 MFCC）。
8. **alignment** (可选): 如果使用对齐信息进行训练，可能包含对齐的索引或其他相关信息。

```json
[
    {
        "id": "sample_001",
        "audio": "/path/to/audio/sample_001.wav",
        "duration": 4.5,
        "text": "Hello, this is a test sentence.",
        "speaker": "speaker_01",
        "language": "en",
        "features": "/path/to/features/sample_001.npy"
    },
    {
        "id": "sample_002",
        "audio": "/path/to/audio/sample_002.wav",
        "duration": 5.0,
        "text": "This is another test sentence.",
        "speaker": "speaker_02",
        "language": "en",
        "features": "/path/to/features/sample_002.npy"
    }
]
...
# 创建一个 Recording 实例
recording = Recording.from_file("path/to/audio/sample.wav")

# 访问录音的信息
print(recording.duration)  # 输出时长
print(recording.sampling_rate)  # 输出采样率
```
...
# 可以创建一个 SupervisionSet 来管理多个 SupervisionSegment
supervision_set = SupervisionSet([supervision])

# 访问 supervision 的信息
print(supervision.text)  # 输出转录文本
print(supervision.start)  # 输出开始时间
...
github官网地址：*[sksq96/pytorch-summary: Model summary in PyTorch similar to `model.summary()` in Keras](https://github.com/sksq96/pytorch-summary)*
...
- 查看模型的各层参数和张量：
  ...
  ![1752905892945](深度学习.assets/1752905892945.png)
  ...
  ![1752906026347](深度学习.assets/1752906026347.png)
...
2. 搜索想要下载的预训练模型
   ...
   ![1753794085560](深度学习.assets/1753794085560.png)
...
正态分布为深度学习提供了一种结构化、可控的随机性，这种随机性是模型能够有效学习、泛化并模拟复杂数据的关键。

方差：反映数据的离散程度，方差越大，波动越大数据越离散；方差越小，波动越小数据更集中。

标准差：方差开根号

![1757827350246](深度学习.assets/1757827350246.png)

![1757827384774](深度学习.assets/1757827384774.png)

| 意义               | 核心思想                         | 应用场景                             |
| :----------------- | :------------------------------- | :----------------------------------- |
| **标准化与可比性** | 统一尺度，消除量纲，实现公平比较 | 数据预处理、特征工程、模型评估       |
| **通用概率计算**   | 提供唯一、通用的概率查询标尺     | 统计分析、假设检验、概率推断         |
| **异常值检测**     | 基于“经验法则”快速识别极端值     | 数据清洗、质量控制、欺诈检测         |
| **简化数学处理**   | 形式最简洁，便于理论推导和证明   | 数学证明、定理表述（如中心极限定理） |
| **深度学习应用**   | 稳定训练，规整潜在空间           | 批量归一化、变分自编码器             |
...
1. 伪造攻击类型：TTS、VC、Replay、Deepfake
2. 伪造源：声码器
3. 对比实验
   - 使用MUSAN和RIRS数据增强 VS SpecAug
   - 简单的分类 VS 复杂的分类
   - 
4. 
...
1. **投影头 & 正负样本选择**：短小的 MLP projection head 常用；positive 可用“同 class 的不同样本”或“同样 condition 但不同噪声配对”。